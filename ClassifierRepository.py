...
class ClassifierRepository(Base):
	...
	def __init__(self,
		     ...
		     port="",
		     dbname=""):
		super()._init__(dbuser, dbpass, dbhost, port, dbname)
	...
	def update(self, classifier = Classifier()):
		...
		"""
		session = self.session_factory()
		classifierDB = session.query(ClassifierDB).filter_by(id=classfier.id).first()
		dic = {}
		if (classifierDB.idPlant != classifier.idPlant):
			dic['idPlant'] = classifier.idPlant
	...
	def search(self, classifier = Classifier(), pageSize = 10, offset = 10):
		"""
